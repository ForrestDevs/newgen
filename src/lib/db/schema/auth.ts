...
export const accounts = pgTable("accounts", {
  ...
  userId: uuid("user_id").references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }).notNull(), // User id
...
export const users = pgTable("users", {
  ...
  emailVerified: timestamp("email_verified", {withTimezone: true, mode: "date"}), // Email verification date
...
export const sessions = pgTable("sessions", {
  ...
  expiresAt: timestamp("expires_at", { withTimezone: true, mode: "date" }).notNull(), // Session expiration
});

export const verificationTokens = pgTable("verification_tokens", {
  id: uuid("id").defaultRandom().primaryKey().notNull(), // Verification token id
  userId: uuid("user_id").references(() => users.id).notNull(), // User id
